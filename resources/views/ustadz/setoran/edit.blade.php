...
<div class="max-w-3xl mx-auto space-y-6"
     x-data="{
         kelancaran: {{ old('nilai_kelancaran', $setoran->evaluasi?->nilai_kelancaran ?? 80) }},
         tajwid: {{ old('nilai_tajwid', $setoran->evaluasi?->nilai_tajwid ?? 80) }},
         makhraj: {{ old('nilai_makhraj', $setoran->evaluasi?->nilai_makhraj ?? 80) }},
         get nilaiAkhir() {
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Edit Setoran</h1>
            <p class="text-sm text-gray-500 mt-0.5">Perbarui data setoran hafalan</p>
        </div>
    </div>

    <form method="POST" action="{{ route('ustadz.setoran.update', $setoran) }}" class="space-y-5">
        @csrf
        @method('PUT')
            ...
            <div>
                ...
                <select id="santri_id" name="santri_id"
                    ...
                    @foreach ($santriList as $santri)
                        <option value="{{ $santri->id }}" {{ old('santri_id', $setoran->santri_id) == $santri->id ? 'selected' : '' }}>
                            {{ $santri->nama_lengkap }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select id="jenis" name="jenis"
                        ...
                        <option value="setoran_baru" {{ old('jenis', $setoran->jenis) === 'setoran_baru' ? 'selected' : '' }}>Setoran Baru</option>
                        <option value="murajaah" {{ old('jenis', $setoran->jenis) === 'murajaah' ? 'selected' : '' }}>Murajaah</option>
                    </select>
                    @error('jenis')
                        <p class="mt-1.5 text-xs text-red-600">{{ $message }}</p>
                    @enderror
            ...
            <div>
                ...
                <select id="surah_id" name="surah_id"
                    ...
                    @foreach ($surahList as $surah)
                        <option value="{{ $surah->id }}" {{ old('surah_id', $setoran->surah_id) == $surah->id ? 'selected' : '' }}>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input id="ayat_awal" type="number" name="ayat_awal" min="1"
                        value="{{ old('ayat_awal', $setoran->ayat_awal) }}"
                        class="w-full px-4 py-2.5 border rounded-lg text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent transition duration-150 {{ $errors->has('ayat_awal') ? 'border-red-400 bg-red-50' : 'border-gray-300' }}">
                    @error('ayat_awal')
                        <p class="mt-1.5 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    ...
                    <input id="ayat_akhir" type="number" name="ayat_akhir" min="1"
                        value="{{ old('ayat_akhir', $setoran->ayat_akhir) }}"
                        class="w-full px-4 py-2.5 border rounded-lg text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent transition duration-150 {{ $errors->has('ayat_akhir') ? 'border-red-400 bg-red-50' : 'border-gray-300' }}">
                    @error('ayat_akhir')
                        <p class="mt-1.5 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <textarea id="catatan" name="catatan" rows="3"
                ...
                class="w-full px-4 py-2.5 border rounded-lg text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent transition duration-150 {{ $errors->has('catatan') ? 'border-red-400 bg-red-50' : 'border-gray-300' }}">{{ old('catatan', $setoran->catatan) }}</textarea>
            @error('catatan')
                <p class="mt-1.5 text-xs text-red-600">{{ $message }}</p>
            @enderror
        </div>
